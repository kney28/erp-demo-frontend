        ...
        <q-card-section>
          <q-form ref="myForm" @submit.prevent="">
            <div class="row justify-around">
              ...
              <div class="col-md-4">
                <q-input filled v-model="day" white color="blue" mask="date" lazy-rules :rules="[val => !!val || 'El campo es obligatorio']">
                  <template v-slot:append>
...
export default defineComponent({
  name: 'HolidayssPage',
  setup () {
    const path = '/configuration/holidays'
    const dialog = ref(false)
    const visible = ref(false)
    const id = ref(null)
    const states = ref(STATUS)
    const filter = ref(null)
    const dataHolidayss = ref([])
    const code = ref(null)
    const day = ref(null)
    ...
    const onDelete = (row) => {
      $q.dialog({
        title: 'Confirmación',
        message: 'Está seguro que desea eliminar el festivo: ' + row.description + '?',
        ok: {
          label: 'Si',
          color: 'positive'
        },
