...
<div>
<q-space />
<q-table dense :rows-per-page-options="[10, 15, 20, 25, 50, 0]" v-model:pagination="pagination" title="Floor" :rows="dataFloors" :filter="filter" :columns="columns" row-key="name" >
...
export default defineComponent({
  name: 'FloorsPage',
  setup () {
    const path = '/hospitalization/floors'
    ...
    const onDelete = (row) => {
      $q.dialog({
        title: 'Confirmación',
        message: '¿Está seguro que desea eliminar el piso: ' + row.description + '?',
        ok: {
          label: 'Si',
          color: 'positive'
        },
