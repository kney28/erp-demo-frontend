  ...
  <q-page class="q-pa-md q-gutter-sm">
    <div>
      <transition appear enter-active-class="animated fadeIn" leave-active-class="animated fadeOut">
        <div>
          <q-space />
          <q-table dense :rows-per-page-options="[10, 15, 20, 25, 50, 0]" v-model:pagination="pagination" title="Tsbanks"
            :rows="dataTsbankss" :filter="filter" :columns="columns" row-key="name">
        ...
        <q-card-section>
          <q-form ref="myForm" @submit.prevent="">
            <div class="row justify-around">
              ...
              <div class="col-md-4">
                <q-input white color="blue" v-model="codeach" label="Código ACH *"/>
              </div>
              <div class="col-md-4"></div>
              <div class="col-md-4">
                <br>
...
export default defineComponent({
  name: 'TsbankssPage',
  setup () {
    const path = '/treasury/tsbankss'
    const dialog = ref(false)
    const visible = ref(false)
    const id = ref(null)
    const filter = ref(null)
    const dataTsbankss = ref([])
    const code = ref(null)
    const description = ref(null)
    const idthird = ref(null)
    const codeach = ref(null)
    const dataThird = ref([])
    ...
    const columns = ref([
      ...
      { name: 'codeach', align: 'center', label: 'Código ACH', field: 'codeach', sortable: true },
    ...
    const getTsbankss = async () => {
      visible.value = true
      const { data } = await api.get(path)
      dataTsbankss.value = data
      visible.value = false
    }
    const creating = () => {
      onReset()
    ...
    const onDelete = (row) => {
      $q.dialog({
        title: 'Confirmación',
        message: '!Está seguro que desea eliminar el banco: ' + row.description + '?',
        ok: {
          label: 'Si',
          color: 'positive'
        },
