      ...
      <transition appear enter-active-class="animated fadeIn" leave-active-class="animated fadeOut">
        <div>
          ...
          <q-table dense :rows-per-page-options="[10, 15, 20, 25, 50, 0]"
            title="Subgruposcups" :rows="dataSubgruposcupss" :filter="filter" :columns="columns" row-key="name">
...
export default defineComponent({
  name: 'SubgruposcupssPage',
  setup () {
    const path = '/hiring/subgruposcupss'
    const dialog = ref(false)
    const visible = ref(false)
    const id = ref(null)
    const states = ref(STATUS)
    const filter = ref(null)
    const dataSubgruposcupss = ref([])
    const code = ref(null)
    ...
    const getSubgruposcupss = async () => {
      visible.value = true
      const { data } = await api.get(path)
      dataSubgruposcupss.value = data
      visible.value = false
    }
    const creating = () => {
      onReset()
    ...
    const onDelete = (row) => {
      $q.dialog({
        title: 'Confirmación',
        message: 'Está seguro que desea eliminar el Subgrupo CUPS: ' + row.id + '?',
        ok: {
          label: 'Si',
          color: 'positive'
        },
