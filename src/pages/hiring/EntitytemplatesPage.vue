      ...
      <transition appear enter-active-class="animated fadeIn" leave-active-class="animated fadeOut">
        <div>
          ...
          <q-table dense :rows-per-page-options="[10, 15, 20, 25, 50, 0]" v-model:pagination="pagination"
            title="Entitytemplate" :rows="dataEntitytemplates" :filter="filter" :columns="columns" row-key="name">
...
export default defineComponent({
  name: 'EntitytemplatesPage',
  setup () {
    const path = '/hiring/entitytemplates'
    const dialog = ref(false)
    const visible = ref(false)
    const id = ref(null)
    const states = ref(STATUS)
    const filter = ref(null)
    const dataEntitytemplates = ref([])
    const code = ref(null)
    ...
    const getEntitytemplates = async () => {
      ...
      dataEntitytemplates.value = data
      visible.value = false
    }
    const creating = () => {
      onReset()
    ...
    const onDelete = (row) => {
      $q.dialog({
        title: 'Confirmación',
        message: 'Está seguro que desea eliminar la Plantilla de Entidad: ' + row.id + '?',
        ok: {
          label: 'Si',
          color: 'positive'
        },
