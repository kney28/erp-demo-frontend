      ...
      <transition appear enter-active-class="animated fadeIn" leave-active-class="animated fadeOut">
        <div>
          ...
          <q-table dense :rows-per-page-options="[10, 15, 20, 25, 50, 0]" v-model:pagination="pagination"
            title="Categoriescups" :rows="dataCategoriescupss" :filter="filter" :columns="columns" row-key="name">
...
export default defineComponent({
  name: 'CategoriescupssPage',
  setup () {
    const path = '/hiring/categoriescupss'
    const dialog = ref(false)
    const visible = ref(false)
    const id = ref(null)
    const states = ref(STATUS)
    const filter = ref(null)
    const dataCategoriescupss = ref([])
    const code = ref(null)
    ...
    const getCategoriescupss = async () => {
      visible.value = true
      const { data } = await api.get(path)
      dataCategoriescupss.value = data
      visible.value = false
    }
    const creating = () => {
      onReset()
    ...
    const onDelete = (row) => {
      $q.dialog({
        title: 'Confirmación',
        message: 'Está seguro que desea eliminar la Categoría CUPS: ' + row.id + '?',
        ok: {
          label: 'Si',
          color: 'positive'
        },
